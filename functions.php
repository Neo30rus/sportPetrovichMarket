...
function createProduct($title, $type_id, $price, $description, $file)
{
    $file_id = null;
    var_dump($file);
    if (!empty($file)) {
    ...
    $product_id = insert("INSERT INTO catalog (title, type_id,price,description,file_id)
                            VALUES (:title, :type_id, :price, :description,:file_id)",
        [
            'title' => $title,
            'type_id' => $type_id,
            'price' => $price,
...
{
    $list = select('SELECT title,description,file_id,catalog.id ,price,name as type FROM  catalog
